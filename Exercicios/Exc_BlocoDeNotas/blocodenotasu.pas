...
var
  ...
  begin
    ...
    begin
      FFoundPos := PosEx(FindText, MBlocoDeNotas.Lines.Text, FFoundPos+1);
      if FFoundPos > 0 then
      begin
        MBlocoDeNotas.SelStart := FFoundPos - 1;
        MBlocoDeNotas.SelLength := Length(FindText);
        MBlocoDeNotas.SetFocus; // Memo1 must be activated, otherwise the selection effect will not be displayed
      end else
        Beep();
    end;
  end;
